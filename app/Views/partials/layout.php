...
<div class="container nav-scroller mt-3 bg-transparent ">
  <nav class="nav nav-underline" aria-label="Secondary navigation">
    ...
    <a class="nav-link active text-primary" aria-current="page" href="<?=base_url()?>"><span class="text-danger"><i class="bi bi-geo-alt"></i> Locations With Jobs:</span> <span class="bg-danger badge p-1 "><?=count($countiesWithJobs)?></span></a>

    <?php  foreach($countiesWithJobs as $cty_wt_job): ?>
      <?php 
            $tj = 0;
            ?>
        <a class="nav-link active" aria-current="page" href="<?=base_url('job/'.$cty_wt_job['location'])?>"><?=$cty_wt_job['location']." "?>:
        <?php foreach($currentJobs as $cj) :?>
          <?php if($cj['location'] == $cty_wt_job['location']){
              $tj += 1;
            }
          ?>
          
        <?php endforeach ?>
        <?php echo "<span class='bg-primary rounded-pill text-white px-1'>".$tj."</span>"; ?>
      </a>  
      </a>
    <?php endforeach ?>
